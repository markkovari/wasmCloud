...
impl Drop for TestWashInstance {
    fn drop(&mut self) {
        ...
        // Attempt to remove the test directory
...
impl TestWashInstance {
    ...
    /// Create a new [`TestWashInstance`] without a nats and connect it to another [`TestWashInstance`]
    pub async fn create_connected(other_insance: TestWashInstance) -> Result<Self> {
        ...
        let mut cmd_args = [
            "up",
            "--nats-port",
            other_insance.nats_port.to_string().as_ref(),
            "--nats-connect-only",
            "--output",
            "json",
            "--detached",
            ...
            host_seed_str,
            "--cluster-seed",
            other_insance.cluster_seed.as_str(),
            "--multi-local",
        ]
        .iter()
        ...
        let test_dir = other_insance.test_dir.clone().unwrap();
        let log_path = test_dir.join("washup_other.log");
        ...
        Ok(TestWashInstance {
            ...
            deployed_wadm_manifest_path,
            nats: None,
            nats_port: other_insance.nats_port,
            host_seed: host_seed_str.into(),
            cluster_seed: other_insance.cluster_seed.clone(),
            host_id,
        })
    }
